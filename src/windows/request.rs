...
use windows::{
    ...
    Win32::{
        Foundation::FALSE,
        Graphics::Gdi::{GetBitmapBits, GetObjectW, BITMAP, BITMAPINFOHEADER, HBITMAP},
        Storage::FileSystem::{
...
use crate::{
    windows::drop::{BitmapDropper, IconDropper},
    Error,
};
...
pub fn get_icon(ext: &str, size: i32) -> Result<Vec<u8>, Error> {
    let mut icon = if ext.to_lowercase().ends_with(".exe") {
        ...
        if icon.is_invalid() {
            if let Some(pos) = ext.find(".exe") {
                icon = get_icon_from_ext(&ext[pos..], size, FILE_ATTRIBUTE_NORMAL);
            } else {
    ...
    unsafe {
        GetObjectW(
            ...
            Some(&mut bmp_color as *mut _ as *mut _),
        )
    };
...
fn get_icon_from_ext(ext: &str, size: i32, attributes: FILE_FLAGS_AND_ATTRIBUTES) -> HICON {
